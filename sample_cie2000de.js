...
function ciede2000(L1,a1,b1, L2,a2,b2) {
    ...
    var tmp_ap_calculate = (1 - Math.sqrt(pow_C_7 / (pow_C_7 + pow_25_7)));

    var ap1 = a1 + (a1 / 2) * tmp_ap_calculate;
    var ap2 = a2 + (a2 / 2) * tmp_ap_calculate;
    var Cp1 = Math.sqrt(Math.pow(ap1, 2) + Math.pow(b1, 2));
    var Cp2 = Math.sqrt(Math.pow(ap2, 2) + Math.pow(b2, 2));
    var Cp_ = (Cp1 + Cp2) / 2;
    var deltaCp = Cp2 - Cp1;
    ...
    var distance_hp1_hp2 = hp2 - hp1;
    var deltahp;
    if (C1 == 0 || C2 == 0) {
        deltahp = 0;
    } else if (Math.abs(distance_hp1_hp2) <= 180) {
        deltahp = distance_hp1_hp2;
    } else if (hp2 <= hp1) {
        deltahp = distance_hp1_hp2 + 360;
    } else {
        deltahp = distance_hp1_hp2 - 360;
    }
    ...
    var Hp_;
    if (Math.abs(distance_hp1_hp2) > 180) {
        Hp_ =  (hp1 + hp2 + 360) / 2
    } else {
        Hp_ = (hp1 + hp2) / 2
    };
    ...
    var pow_50_2 = Math.pow(L_ - 50, 2);
    var SL = 1 + ((0.015 * pow_50_2) / Math.sqrt(20 + pow_50_2));
    var SC = 1 + 0.045 * Cp_;
